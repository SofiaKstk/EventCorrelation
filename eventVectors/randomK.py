...
# GET BINARY STREAMS
vectors = pd.read_csv("cusum/cusumEventVector.csv", header=None)
# vectors = pd.read_csv("shewhart/shewhartEventVector.csv", header=None)
...
for i in range(1,8):
	...
	for j in vectors:
		...
		if len(ones)>k:
			...
			event = ''.join(event)
		events.append(event)
	file = 'cusum/cusumRandomKevents'+str(k)+'.csv'
	# file = 'shewhart/shewhartRandomKevents'+str(k)+'.csv'
